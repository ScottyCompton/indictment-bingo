...
const PreLoadScreen:React.FC<PreloadScreenProps> = (props:PreloadScreenProps) => {
    ...
    const doPreload = useCallback(() => {
        ...
        const updateStatus = (status: {statusText: string, statusVal: number}, idx:number) => {
            ...
            setBtnLoadingText(`Stand By - ${status.statusVal}% complete` )
        }
    ...
    return (
        <div className={rootClass}>
            ...
            {!loaded && <button id="btnLoadBingo" disabled={true} className="btn btn-secondary form-control big-button">{btnLoadingText}</button>}
        </div>
    )
}
